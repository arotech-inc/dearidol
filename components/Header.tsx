...
export default function DearIdolHeader() {
  ...
  return (
    ...
    >
      ...
      {/* 📱 모바일 햄버거 버튼 */}
      <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden text-white text-3xl z-50 focus:outline-none">
        {isMobileMenuOpen ? "✕" : "☰"}
      </button>
      ...
      {/* 📱 모바일 드롭다운 메뉴 */}
      {isMobileMenuOpen && (
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="absolute top-full left-0 w-full bg-black/95 border-t border-pink-500/30 flex flex-col items-center py-8 gap-8 md:hidden shadow-2xl z-[90]">
          {menuItems.map((item, i) => (
